...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __init__(self, module: FSDP, inference_engine: LLM, model_config, full_params: bool = False, device_mesh: DeviceMesh = None,
                  offload_param: bool = False, load_format: str = "dummy_hf", layered_summon: bool = True, fake_quant_weight: bool = False):
        self.module = module
        ...
        self.fake_quant_weight = fake_quant_weight
    ...
    def update_params(self, updated_params, peft_config=None):
        """
        Load weights into vLLM model for rollout, automatically applying fake quantization
        for StepAwareFakeLinear modules.
        """
        ...
        def maybe_fake_quant(name, param):
            ...
            if name.endswith("weight"):
                ...
                module_names = name.split(".")[:-1]  # 去掉"weight"
                sub_module = getattr(self.module, "_fsdp_wrapped_module", self.module)
                for mn in module_names:
                    sub_module = getattr(sub_module, mn, None)
                    if sub_module is None:
                        break

                if isinstance(sub_module, StepAwareFakeLinear):
                    param = sub_module.fake_quant_weight(
                        param.to(device),
                        soft_round_enable=bool(sub_module.soft_round_enable.item()),
                        progressive_enable=bool(sub_module.progressive_enable.item()),
                        progressive_ratio=sub_module.progressive_ratio.item(),
                    )
                    print(f"[Rollout Quant] {name} quantized")

            return param.to(device, non_blocking=True)

        # 构造生成器，只保留不为 None 的参数
        loaded_params = model.load_weights(
            ((name, p) for name, param in updated_params.items() if (p := maybe_fake_quant(name, param)) is not None)
        )
