...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    wbits: int = field(
        default=4,
        metadata={"help": "How many bits to start."}
    )
    tbits: int = field(
        default=2,
        metadata={"help": "How many bits to end."}
    )
    group_size: int = field(
        default=128,
        metadata={"help": "How many group size to use."}
    )
    pg_duration: int = field(
        default=500,
        metadata={"help": "How many steps to arrive target."}
    )
...
def get_accelerate_model(args, checkpoint_dir):
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.quant_model_path, use_fast=False)
    ...
    load_checkpoint_in_model(
        model,
        checkpoint=args.quant_model_path,
...
def train():
    ...
    quant_controller = QuantizationController({})
    quant_controller.enable = args.wbits < 16
    quant_controller.start = args.wbits
    quant_controller.target = args.tbits
    quant_controller.pg_duration = args.pg_duration
    trainer = Seq2SeqTrainer(
        model=model,
        tokenizer=tokenizer,
        args=training_args,
        optimizers=(optimizer, None),
        callbacks=[QuantizationCallback(quant_controller)],
